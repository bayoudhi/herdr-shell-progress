...
mod tests {
    ...
    fn report_agent_omits_message_when_absent() {
        ...
        assert!(v.get("message").is_none(), "absent message must not be sent");
    }
    ...
    fn report_metadata_includes_ttl_when_given() {
        let v = report_metadata("w1:p2", None, Some(("idle", "ok · 4s".to_string())), Some(20_000), false);
    ...
    fn report_metadata_clear_sets_both_clear_flags() {
        let v = report_metadata("w1:p2", None, None, None, true);
    ...
    fn envelope_is_one_newline_terminated_json_line() {
        ...
        assert_eq!(line.matches('\n').count(), 1, "exactly one line per request");
